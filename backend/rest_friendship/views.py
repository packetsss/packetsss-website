...
PERMISSION_CLASSES = [import_string(c) for c in REST_FRIENDSHIP["PERMISSION_CLASSES"]]
...
class FriendViewSet(viewsets.ModelViewSet):
    ...
    def list(self, request):
        requested_user = Friend.objects.friends(user=request.user)
        self.queryset = requested_user
        self.http_method_names = [
            "get",
            "head",
            "options",
        ]
        return Response(FriendSerializer(requested_user, many=True).data)
    ...
    def add_friend(self, request, username=None):
        ...
        to_user = get_object_or_404(
            User, username=serializer.validated_data.get("to_user")
        )
